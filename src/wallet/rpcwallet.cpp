...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toCFLaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("autocombinerewards")) {
        if (request.fHelp) {
            throw std::runtime_error("autocombinerewards (Deprecated, will be removed in a future update. To use this command, start cryptoflowd with -deprecatedrpc=autocombinerewards)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "autocombinerewards is deprecated and will be removed in a future update. To use this command, start cryptoflowd with -deprecatedrpc=autocombinerewards");
    }
...
UniValue setautocombinethreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.empty() || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"threshold\": n.nnn,         (numeric) auto-combine threshold in CFL\n"
...
UniValue getautocombinethreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            ...
            "{\n"
            "  \"enabled\": true|false,        (boolean) true if auto-combine is enabled, otherwise false\n"
            "  \"threshold\": n.nnn            (numeric) the auto-combine threshold amount in CFL\n"
            "}\n"
