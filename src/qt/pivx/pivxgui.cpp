...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("CryptoFlow client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("CryptoFlow"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("CryptoFlow"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
