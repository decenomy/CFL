...
{
	RenameThread("cryptoflow-scriptch");
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
	...
	// Check freeze point
    if (block.GetBlockTime() > 1615852800) // Tue Mar 16 2021 00:00:00 GMT+0000
    {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
	...
	if (block.IsProofOfStake() && nHeight > DEFAULT_BLOCK_SPAM_START && IsSporkActive(SPORK_18_ACCEPT_BLOCK_PATCH_ENFORCEMENT)) {
		LOCK(cs_main);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	while (true) {
		...
		if (pfrom && GetBoolArg("-blockspamfilter", DEFAULT_BLOCK_SPAM_FILTER)) {
			...
			if (chainActive.Height() > DEFAULT_BLOCK_SPAM_START && IsSporkActive(SPORK_17_BLOCK_PATCH_ENFORCEMENT)) {
