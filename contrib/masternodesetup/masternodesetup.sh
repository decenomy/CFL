...
COIN_TGZ=`curl -s https://api.github.com/repos/CryptoFlowCoin/CryptoFlowCoin/releases/latest | grep "browser_download_url.*Linux\\.zip" | cut -d : -f 2,3 | tr -d \" | xargs`
...
libboost-system-dev libboost-test-dev libboost-thread-dev sudo automake git wget curl net-tools libdb4.8-dev bsdmainutils libdb4.8++-dev \
...
libboost-program-options-dev libboost-system-dev libboost-test-dev libboost-thread-dev sudo automake git curl net-tools libdb4.8-dev \
